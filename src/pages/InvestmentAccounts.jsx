...
function InvestmentAccounts() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://localhost:3001/getInvestmentAccount');
        setUsers(response.data);
      } catch (err) {
        console.error(err);
      }
  ...
  const handlePrint = () => {
    const table = document.getElementById('accounts-table').outerHTML;
    const originalContents = document.body.innerHTML;
    ...
    // Restore event listeners after reloading the content
    window.location.reload();
  };
